...
---
# <a name="render-a-card---ios"></a>Rendu d’une carte iOS

Voici comment afficher une carte à l’aide du kit de développement logiciel (SDK) iOS.

## <a name="create-a-card-from-a-json-string"></a>Créer une carte à partir d’une chaîne JSON

AdaptiveCard est généré à partir d’une chaîne JSON
...
Retournée UIView utilise la disposition automatique. La largeur sera contrainte à la valeur définie par widthConstraint. Si la valeur 0 est utilisée, elle ne sera pas liée.
La hauteur n’est pas liée, et lorsqu’elle est retournée, elle affiche la hauteur des sommes de tout le contenu rendu. Pour lier la dimension de vue, utilisez NSLayoutConstraint. La dimension exacte est accessible à partir du contexte de viewDidLayoutSubview de son ViewController d’affichage ou de sa méthode avec le même nom si ACRViewController est utilisé.
